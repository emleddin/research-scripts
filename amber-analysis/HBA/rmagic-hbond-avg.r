## Run this with "Rscript rmagic-hbond-avg.r"
...
A_avg <- "/absolute/path/to/the/avgeraging/output/WT_protein_system_total_hbond_avg.dat"

## Explicitly set the number of the SNP/changed residue to match
fix_string <- "_000"
fixed_string <-"Changed_Res"
...
## Change the SNP or Varied position name in Acceptor with something checkable
findmeAcc <- grep(fix_string, save_cols_AH$Acceptor)
save_cols_AH$Acceptor <- replace(save_cols_AH$Acceptor, findmeAcc, fixed_string)

## Change the SNP or Varied position name in Donor with something checkable
findmeDon <- grep(fix_string, save_cols_AH$Donor)
save_cols_AH$Donor <- replace(save_cols_AH$Donor, findmeDon, fixed_string)
